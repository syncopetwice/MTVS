...
(function ($, _, undefined) {
  ...
  //Updates the values of the main variables
    function updateValues() {
      ...
      _.each(mentionsCollection, function (mention) {
        ...
        var textSyntax = mention.type == 'user' || mention.type == 'group' ? settings.templates.mentionItemSyntax(formattedMention) : settings.templates.hashItemSyntax(formattedMention);
  ...
  //Adds mention to mentions collections 
    function addMention(mention, chBrackets) {
      ...
      var currentLength = currentDataQuery.length;
      if(charVariable == '#'){
      ...
      var startCaretPosition = lIndex - currentLength - 1; //Set the star caret position
  ...
  //Takes the keydown event
    ...
    if(e.keyCode === KEY.SPACE && charVariable == '#'){
    countWords++;
  ...
  //Populates dropdown
    ...
    if(currentDataQuery.length<1)hideAutoComplete();
  ...
  //Resets the text area
    function resetInput() {
      if(settings.editMessage.length){
        elmInputBox.val(settings.editMessage);
      }else{
        elmInputBox.val('');
      }
      if(settings.prefillMentions.length){
        mentionsCollection = settings.prefillMentions;
      }else{
        mentionsCollection = [];  
      }
      updateValues();
